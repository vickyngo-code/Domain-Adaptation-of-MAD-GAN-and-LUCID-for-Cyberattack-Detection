...
def cicddos(seq_length, seq_step, num_signals, randomize=False):
    ...
    # assume no cuts off in the beginning
    samples = train[:, 0:n - 1]
    ...
    ####################################
    ###################################
    # -- the best PC dimension is chosen pc= ?-- #
...
def cicddos_test(seq_length, seq_step, num_signals, randomize=False):
    """ Load and serialise """
    # test = np.load('./data/swat_a.npy')
    # print('Loaded swat_a from .npy')
    test = np.loadtxt(open('./data/cicddos2019_atk.csv'), delimiter=',')
    print('Loaded CICDDoS2019 from .csv')
...
def swat(seq_length, seq_step, num_signals, randomize=False):
    ...
    for i in range(n - 1):
        ...
        if A != 0:
            ...
            train[:, i] = train[:, i]
    samples = train[0:, 0:n-1]
    labels = train[0:, n-1]    # the last colummn is label
    #############################
    # -- choose variable for uni-variate GAN-AD -- #
...
def get_data(data_type, seq_length, seq_step, num_signals, sub_id, eval_single, eval_an, data_options=None):
    ...
    if data_type == 'load':
    	...
    	samples, labels = cicddos(seq_length, seq_step, num_signals)
    elif data_type == 'cicddos2019':
    	samples, labels, index = cicddos_test(seq_length, seq_step, num_signals)
    else:
